...
func (o *DescribeWorkspaceURL) Build() (*url.URL, error) {
	...
	var _path = "/tf/workspace/{name}"

	name := o.Name
	if name != "" {
		_path = strings.Replace(_path, "{name}", name, -1)
	} else {
		return nil, errors.New("Name is required on DescribeWorkspaceURL")
	}
...
func (o *DescribeWorkspaceURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DescribeWorkspaceURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DescribeWorkspaceURL")
	}
