...
func (o *UndeployStackURL) Build() (*url.URL, error) {
	...
	var _path = "/tf/stack/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("ID is required on UndeployStackURL")
	}
...
func (o *UndeployStackURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UndeployStackURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UndeployStackURL")
	}
